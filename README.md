...
Die [Android-Anleitung](_site-src/docs/android.md) führt von Installation und erster Anmeldung über die Galerie bis zur Personenverwaltung. Build, API und Datenhaltung stehen in der [technischen Referenz](_site-src/docs/android-technik.md).

**0.68.3** schützt Bestandsdaten in Upload-Duplikatantworten und Dokument-Tags vor Konten ohne Dokument-Leserecht. Der Mail-Import prüft die Nachrichtengröße vor dem IMAP-Download, begrenzt den angeforderten Inhalt und akzeptiert höchstens 32 verschachtelte MIME-Container. Überlange oder zu tief verschachtelte Nachrichten bleiben mit Fehlerstatus im Postfach. Sicherheitskorrekturen ohne Datenmigration.
...
**0.61.3** korrigiert die fehlenden Bildvorschauen beim Benennen/Zuordnen: Der Zusammenführungsdialog zeigt beide Vergleichsgesichter mit Ordnernamen, der Dialog für eine einzelne Gruppe ihr jeweiliges Gesicht.
...
Voraussetzung fuer Entwicklung und Build ist Go `1.27.1` oder eine kompatible neuere Version. Fuer `make test-js` wird zusaetzlich `node` benoetigt. Die optionalen Playwright-Smokes brauchen zusaetzlich `npm` und einen Playwright-kompatiblen Browser; lokal ist standardmaessig der Chrome-Channel konfiguriert.
...
[Personenanleitung](_site-src/docs/fotos-personen.md#personen-sortieren).
...
Auch bei mehr als 50 passenden Ordnern liefert die Suche alle Treffer mit korrekter Gesamtzahl und Sortierung; die App lädt weiterhin höchstens 24 Ordner mit bis zu vier Vorschauen (virtuelle Personenordner bis zu acht) pro Abruf.
...
Ab App **0.9.0** und BearStack **0.49.0** zeigt **Menü → Ähnliche Gruppen** immer ein Gruppenpaar als einzelne Entscheidung. **Zusammenführen** oder **Getrennt lassen** speichert die Entscheidung und lädt automatisch das nächste Paar; beide Buttons bleiben ohne Scrollen erreichbar. Beide Portraits bieten die Originalfoto-Vergrößerung per Halten und Wischen wie beim Benennen. Über jedem Portrait steht der enthaltende Ordner des Vergleichsfotos statt „Erste Gruppe“ oder „Zweite Gruppe“; fehlt der Bildpfad, bleibt die bisherige Gruppenbeschriftung. Beide Portraits stehen gleich groß auf derselben Höhe; Namen und Gesichtsanzahlen sind darunter ausgerichtet. Ignorieren und Stift stehen nebeneinander unter der jeweiligen unbenannten Gruppe. Größere vertikale Abstände gliedern Portraits, Details und Aktionen; gemeinsame Benennungsvorschläge setzen sich mit zusätzlichem Abstand und großzügigeren Trefferkarten ab. Bei wenig Platz oder großer Schrift scrollt nur der Vergleichsbereich, während die Entscheidungsbuttons unten sichtbar bleiben. Gruppenrevisionen und dauerhaft gespeicherte Aktionsquittungen sichern Konflikte und Verbindungsabbrüche ab. Die Benennen-Warteschlange bleibt erhalten; der vorhandene begrenzte Originalbildcache wird wiederverwendet.

Beim **Halten eines Gesichts** lädt die Android-App das ganze Foto als große WebP-Vorschau gemäß **`large_preview_size`** vom Server (Standard: 3840 Pixel längste Kante). Das gilt auch für das WLAN-Vorladen in Benennen, Personenverwaltung und Ähnliche Gruppen. Der Server verwendet den bestehenden Thumbnail-Cache; die App behält ihr Dekodierziel von 2048 Pixeln sowie den Drei-Minuten-/16-MiB-Speichercache. App und Server gemeinsam aktualisieren: ältere Server ohne diesen Modus liefern weiterhin die Originaldatei. Teilen verwendet weiterhin das Original.

Im Android-Modus **Ähnliche Gruppen** startet der automatische Lupenabgleich nur, wenn **beide Gruppen unbenannt** sind. Zuerst wird das Vergleichsgesicht von **Gruppe 1** geprüft. Bleibt das abgeschlossene Ergebnis leer, wird anschließend **Gruppe 2** abgeglichen. Daraus entstehen gemeinsame **Benennungsvorschläge für beide Gruppen** unter Ignorieren und Stift. Treffer mit Referenzportrait sind bereits während der Suche wählbar; Antippen ordnet **beide Gruppen atomar** derselben Person zu und lädt nach Bestätigung das nächste Paar. Ein gemeinsamer Lade-, Leer- oder Fehlerzustand erlaubt eine gezielte Wiederholung; höchstens zwei aufeinanderfolgende Suchanfragen und 20 Treffer, keine automatischen Fehlerschleifen. Fertige Ergebnisse bleiben für das aktuelle Paar erhalten; Einzelaktionen verwerfen die gemeinsame Liste. Revisionsprüfungen beider Gruppen und der Zielperson sowie Aktionsquittungen sichern Konflikte und verlorene Antworten ab.

Mit gespeicherten Verbindungsdaten zeigt die Android-App beim Start einen BearStack-Ladebildschirm und meldet sich automatisch an. **Lokale Fotos öffnen** ist schon währenddessen verfügbar. Scheitert die Anmeldung oder antwortet der Server innerhalb von acht Sekunden nicht, öffnet sich die lokale Galerie mit Fehlerhinweis und **Erneut versuchen**; Zugangsdaten und Zertifikatsbindung bleiben erhalten. Gerätefotos funktionieren auch ohne Servereinrichtung und unabhängig vom Schalter für die lokale Ordnerkachel, sobald Android den Fotozugriff erlaubt. Eine erneute Anmeldung erhält die offene lokale Ansicht. **Gruppen manuell kombinieren** wurde aus der App entfernt; Ähnliche Gruppen und die Wiederaufnahme alter offener Aktionen bleiben erhalten.
