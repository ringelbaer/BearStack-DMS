...
`internal/server/services.go` bündelt den Aufbau von Dokumentimport, Nachverarbeitung, OCR, Thumbnails, Office-Vorschauen, Mailimport, Papierkorb und Gesichtsanalyse. `Server.New` initialisiert diese Services vor der Freigabe des Servers. Teilweise aufgebaute Test-Server verwenden beim ersten Zugriff denselben durch `sync.Once` geschützten Aufbau. Vorgegebene Services werden vor dem ersten Zugriff eingesetzt und bleiben erhalten. Konstruktion erzeugt nur Instanzen und Kanäle; Start und Shutdown der Worker bleiben im vorhandenen Hintergrund-Lifecycle.

Ab 0.66.2 besitzt `face_analysis_service.go` die gemeinsame abbrechbare Analysesperre und die Client-Erzeugung. Hintergrundlauf und manuelle Analyse verwenden denselben Ablauf aus Bildaufbereitung, Inferenz, Ausschnittverbesserung und Speichern über eine schmale Fotoschnittstelle. Die Quellenprüfung nutzt Inferenz ohne Ersetzen vorhandener Gesichter; ihre Revisionsprüfung und Bestätigung bleiben im bisherigen Ablauf. Die Aufrufer halten die gemeinsame Sperre von der Jobauswahl beziehungsweise Revisionsprüfung bis zur abschließenden Änderung; auch das Leeren der Gesichtsdaten verwendet sie. Worker behalten einen Client pro Batch, HTTP-Aktionen einen pro Aufruf und schließen jeweils dessen ungenutzte Verbindungen. Zeitplanung, Berechtigungen und HTTP-Fehlerantworten bleiben bei Worker beziehungsweise Handler.

Die fachliche Mail- und OCR-Ablaufsteuerung liegt in `internal/mailservice` und `internal/ocrservice`. Beide Pakete verwenden eigene schmale Schnittstellen für Repository, Storage und Importer beziehungsweise OCR-Engine; sie importieren weder das HTTP-Paket noch konkrete Repository-Implementierungen. Der Server übernimmt Konstruktion, Audit-Anbindung und den gemeinsamen Worker-Lifecycle. Mail-Einstellungsregeln liegen im Dokumentmodell, Importfehlertexte beim Dokumentimport.

`internal/documentformat` enthält die gemeinsamen Formatregeln. MIME-Typen werden ohne Parameter, mit bereinigten Leerzeichen und unabhängig von Groß-/Kleinschreibung ausgewertet. Der Renderer und der einmalig erzeugte SQL-Filter für Thumbnail-Kandidaten nutzen dieselben Definitionen. Die Datenbank filtert weiterhin vor `LIMIT` und blättert über unveränderliche IDs; zusätzliche Datenbankspalten oder Migrationen sind dafür nicht erforderlich.
...
Der Ordnerdienst in `document_services.go` hängt seit 0.66.2 nur von vier lesenden Repository-Methoden ab: Tags, Feldwerte, Suchfavoriten und Dokumentanzahl. Der Server stellt konkrete Datenquelle, Caches und gebündelte Zählung bereit. HTML, API und WebDAV teilen die Ordnerregeln; Root-Tags werden weiterhin wiederverwendet und Favoriten zusammen gezählt. Dadurch sind Regeln und Abfragebudgets ohne SQLite prüfbar.
...
`app-person-picker.js` besitzt seit 0.66.2 ausschließlich Suche, Vorschlagslisten, Vorschauen und Auswahl per Tastatur oder Touch. `BearStackPersonPicker.bind` erhält Suchkontext und Auswahl-Callbacks; die zurückgegebene Instanz unterstützt Auslesen, Schließen und Zurücksetzen der Auswahl. `app-person-dialog.js` verbindet diese Komponente mit Formularzielen, Validierung, Buttontexten und Absenden. Die Auswahl selbst verändert keine Formularaktion und sendet kein Formular ab. Der Kern wird vor dem Dialogadapter geladen; Abbruch veralteter Anfragen, begrenzte Ergebnislisten und gebündeltes Rendering bleiben erhalten.
