...
Reine Testhelfer bleiben in `_test.go`-Dateien. Ab 0.52.1 gilt das auch für `nearestPerson`, `decodeGPX` und `normalizeRenderSettingsSnapshot`; ihre bestehenden Aufrufer und Regressionen bleiben erhalten. Das getrennte Blogdarstellungspaket prüft zusätzlich, dass HTML-Eingaben maskiert werden, Textdateien ihre Leerzeichen behalten und ungültige Datumsangaben entfallen. Ab 0.41.2 liegen auch die Helfer zum Vorbelegen der Einstellungs-Caches in `internal/server/test_helpers_test.go`. Mailimport-Tests rufen die gemeinsamen Anhangsfunktionen direkt auf; die ausschließlich in Tests genutzten PDF-Weiterleitungen sind entfernt. Absenderfilter, Größenlimit, Decodierung und sichere Dateinamen bleiben durch dieselben Prüfungen abgedeckt.
