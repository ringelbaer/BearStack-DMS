...
var photoSchemaMigrations = []photoSchemaMigration{
	...
	{Version: 21, Name: "expiring ignored face thumbnails"},
	...
	{Version: 38, Name: "person folder exclusions"},
	{Version: 39, Name: "family tree root selection"},
	{Version: 40, Name: "image groups and primary images"},
}
...
func runPhotoSchemaMigrations(ctx context.Context, db *sql.DB) error {
	...
	for _, migration := range photoSchemaMigrations {
		...
		if migration.Version == 40 {
			if err := setupImageGroups(ctx, db); err != nil {
				return err
			}
			continue
		}
		if migration.Version == 39 {
			if err := setupFamilyTreeSchema(ctx, db); err != nil {
				return err
			}
			continue
		}
		if migration.Version == 38 {
			if err := setupPersonFolderSchema(ctx, db); err != nil {
				return err
			}
			continue
		}
		...
		if migration.Version == 35 {
			if err := setupPersonDetails(ctx, db); err != nil {
				return fmt.Errorf("photo schema migration 35: %w", err)
			}
			continue
		}
		if migration.Version == 33 {
			if err := setupPersonParents(ctx, db); err != nil {
				return fmt.Errorf("photo schema migration 33: %w", err)
			}
			continue
		}
		if migration.Version == 32 {
			if err := setupPersonTags(ctx, db); err != nil {
				return fmt.Errorf("photo schema migration 32: %w", err)
			}
			continue
		}
		...
		if migration.Version == 30 {
			if err := setupFaceThresholds(ctx, db); err != nil {
				return fmt.Errorf("photo schema migration 30: %w", err)
			}
			continue
		}
		if migration.Version == 29 {
			if err := setupFaceDrawnSchema(ctx, db); err != nil {
				return fmt.Errorf("photo schema migration 29: %w", err)
			}
			continue
		}
		if migration.Version == 28 {
			if err := setupFaceAssignmentStats(ctx, db); err != nil {
				return fmt.Errorf("photo schema migration 28: %w", err)
			}
			continue
		}
		...
		if migration.Version == 24 {
			if _, err := db.ExecContext(ctx, `CREATE INDEX IF NOT EXISTS idx_photo_people_named_id ON photo_people(id) WHERE name<>''`); err != nil {
				return err
			}
			continue
		}
		if migration.Version == 23 {
			if err := setupGroupPhotos(ctx, db); err != nil {
				return fmt.Errorf("photo schema migration 23: %w", err)
			}
			continue
		}
		if migration.Version == 22 {
			if err := setupFaceFavorites(ctx, db); err != nil {
				return fmt.Errorf("photo schema migration 22: %w", err)
			}
			continue
		}
		if migration.Version == 21 {
			if err := setupFaceThumbnailSchema(ctx, db); err != nil {
				return fmt.Errorf("photo schema migration 21: %w", err)
			}
			continue
		}
		...
		if migration.Version == 18 {
			if err := setupFaceSchema(ctx, db); err != nil {
				return fmt.Errorf("photo schema migration 18: %w", err)
			}
			continue
		}
